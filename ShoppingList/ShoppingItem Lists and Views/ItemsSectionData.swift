...
import Foundation
	...
	//
	// one thing that's specific to this app ... i've added an index value for each section's data, to
	// indicate whether it's section 1, or 2, or ...  there's no real need to do this in general, but i
	// have placed two buttons on the first section in the ShoppingListView and the PurchasedItemsView
	// to choose between one section for all items, or multiple sections.  so the header for the first
	// section must recognize "i am the first section."  it's a little cheesey ...
	//
struct ItemsSectionData: Identifiable, Hashable {
	var id: Int { hashValue } // so, this will work just like id: \.self for SwiftUI
