...
* This project is now closed.  See the newer ShoppingList16 project released on 2 March, 2023.

> There are 3 branches in this project, which occasionally get bolluxed up when changing branches because of file conflicts in the .xcuserstate file. My advice: to work with this, pick the branch you want to play with, then just download the project as a zip file (so as not to bring along all the source control data).
...
### Most Recent Update(s) of 29 July, 2022

* (*29 Jul*) You are currently on the `master` branch of the SL15 project.  
    - Development on this branch has stopped.  
    - A second branch named `MVVM` implements a different architecture (yes, model - view - view model) in which there is a centralized DataManager class.  This does not use `@FetchRequest`s scattered about in SwiftUI views, but instead moves their functionality into the DataManager class using `NSFetchedResultsController` objects.  There are no global singletons in this architecture; and `Item` and `Location` objects are treated as read-only objects in SwiftUI views (a SwiftUI view cannot directly change objects of either kind unless it either goes through the DataManager, or it cheats and reaches directly into Core Data to do it).
    - A third branch named `MVVM-2` throws a new twist at you, based on the `MVVM` branch.  The DataManager does not vend Core Data objects to views, but instead vends `struct` representations of those objects to views.  In effect, SwiftUI views are being given exactly what they want and for which SwiftUI was designed: `struct` data that is read-only.  That tends to take care of all those "view does not update" subtleties that continue to puzzle programmers who work with Core Data objects in SwiftUI views.
* (*29 Jul*) If you like to use `@FetchRequest`s, stick with the `master` branch.  If you want more of an MVVM architecture, use either the `MVVM` branch or the `MVVM-2` branch (  the latter of these two is where all active work is being done).
